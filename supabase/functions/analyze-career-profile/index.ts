...
serve(async (req) => {
  ...
  try {
    ...
    if (isHealthRequest(body)) return healthResponse("analyze-career-profile", aiProviderChecks(), corsHeaders);
    const { profile, practiceHistory } = body as { profile: any; practiceHistory?: any };
